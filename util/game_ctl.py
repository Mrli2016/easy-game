from util.image_proc import match_img_knn
import configparser
import ctypes
import logging
import os
...
from util.img_match import search_img
from util.tools import CommonPos
...
class GameControl:
    ...
    def window_part_shot(self, pos1, pos2, file_name=None, gray=0):
        ...
        memdc.BitBlt((0, -35), (w, h), srcdc,
                     (pos1[0] + self._border_l, pos1[1] + self._border_t),
                     win32con.SRCCOPY)
        if file_name is not None:
    ...
    def find_img(self, img_template_path, part=0, pos1=None, pos2=None, gray=0, img=None):
        """
        查找图片
    ...
    def find_img_knn(self, img_template_path, part=0, pos1=None, pos2=None, gray=0, threshold=0.9, debug=False):
        """
        查找图片，knn算法
            :param threshold: 相似度
            :param debug:
        ...
        try:
            max_loc = search_img(img_src, img_template, debug=debug, threshold=threshold, gray=gray)
            return max_loc
        except Exception as err:
            logging.warning(f'find_img_knn执行失败[{img_template_path}]:', err)
            a = traceback.format_exc()
            logging.warning(a)
            return None
    ...
    def mouse_click_bg(self, pos, pos_end=None):
        """
        后台鼠标单击
            :param pos: (x,y) 鼠标单击的坐标（左下角）
            :param pos_end: 鼠标点击的右下角
        ...
        if pos_end is None:
            # 默认增加小范围偏移量
            pos_rand = (random.randint(
                pos[0], pos[0] + 5), random.randint(pos[1], pos[1] + 5))
        else:
            pos_rand = (random.randint(
    ...
    def public_task(self, auto_ready=False):
        """处理公共事务，包括悬赏"""
        base_img = f"./images/public/"  # 储存的图片文件夹

        target = self.find_img_knn(base_img + "xuanshang.png")
        if target:
            # 出现悬赏 进行判断处理
            money = self.find_img_knn(base_img + "money.png")
            if money:
                # 如果是勾玉悬赏，进行接受
                target = self.find_img_knn(base_img + "accept.png")
                if target:
                    self.mouse_click_bg(target)
            else:
                target = self.find_img_knn(base_img + "refuse.png")
                if target:
                    self.mouse_click_bg(target)

        if auto_ready:  # 自动准备
            target = self.find_img_knn(base_img + "ready.png")
            if target:
                self.mouse_click_bg(target)

        target = self.find_img_knn(base_img + "win.png")
        if target:
            self.mouse_click_bg(*CommonPos.second_position)
        target = self.find_img_knn(base_img + "over.png")
        if target:
            self.mouse_click_bg(*CommonPos.second_position)
...
def show_img(img):
    ...
    # cv2.imwrite(r"F:\code\yys\images\jiejie\test.png", img)
    cv2.waitKey(0)
