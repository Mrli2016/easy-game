        ...
        for item in vars(CommonPos).items():
            if not '__' in item[0]:
                setattr(CommonPos, item[0], ((
                                                 item[1][0][0], item[1][0][1] + 35),
                                             (item[1][1][0], item[1][1][1] + 35)))
...
class PublicPos():
    ...
    @staticmethod
    def InitPosWithClient__():
        for item in vars(PublicPos).items():
            if not '__' in item[0]:
                setattr(PublicPos, item[0], ((
                                                 item[1][0][0], item[1][0][1] + 35),
                                             (item[1][1][0], item[1][1][1] + 35)))


class YuhunPos:
    fight_btn = (995, 533), (1055, 595)  # 御魂挑战按钮
...
class ActivePos:
    tiaozhan_btn = (1045, 533), (1045, 600)  # 活动挑战按钮

    @staticmethod
    def InitPosWithClient__():
        for item in vars(ActivePos).items():
            if not '__' in item[0]:
                setattr(ActivePos, item[0], ((
                                                 item[1][0][0], item[1][0][1] + 35),
                                             (item[1][1][0], item[1][1][1] + 35)))
